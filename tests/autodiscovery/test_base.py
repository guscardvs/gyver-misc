...
def test_make_modulename_converter(sample_root_path):
...
def test_auto_discovery_helper_excludes(sample_root_path):
    helper = AutoDiscoveryHelper(
        root=sample_root_path,
        exclude=['exclude_dir'],
    )
    excludes = helper._excludes()
    assert 'exclude_dir' in excludes[0]


def test_auto_discovery_helper_includes(sample_root_path):
    helper = AutoDiscoveryHelper(root=sample_root_path, include=['include_dir'])
    includes = helper._includes()
    assert 'include_dir' in includes[0]


def test_auto_discovery_helper_should_look(sample_root_path, tmp_path):
    helper = AutoDiscoveryHelper(
        root=sample_root_path,
        exclude=['exclude_dir'],
        include=['file.py'],
    ...
    assert helper._should_look(path_to_check)

    exclude_path = tmp_path / 'exclude_dir'
    assert not helper._should_look(exclude_path)

    not_included = tmp_path / 'not_included.py'
    assert not helper._should_look(not_included)


def test_sort_files_by_dependency(sample_root_path, sample_files):
    converter = make_modulename_converter(sample_root_path, pathlib.Path.as_posix)
...
def test_ordered_mod(sample_root_path):
    converter = make_modulename_converter(sample_root_path, pathlib.Path.as_posix)
